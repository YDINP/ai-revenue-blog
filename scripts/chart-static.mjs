...
// 그룹 막대: 지표를 x축에 나열하고 두 시리즈를 같은 바닥선에서 위로 세운다.
// 대칭 막대는 라벨을 중심으로 좌우 "반대 방향"으로 자라 공통 기준선이 없었다
// → 두 값의 길이를 눈으로 겹쳐 볼 수 없었다. 높이가 곧 값, 이웃 차이가 곧 격차다.
function versusGrouped(a, items) {
  const title = a.title || '';
  ...
  const H = (v) => (top > 0 ? (v / top) * 100 : 0);
  // 값 자릿수가 많으면 이웃 막대의 값 라벨과 가로로 부딪힌다 → 글자를 줄인다
  const wide = items.some((it) => String(it.a).length > 4 || String(it.b).length > 4);

  let h = title ? `<div class="chart-title">${title}</div>` : '';
  h += `<div class="gb-legend"><span class="gb-key"><i class="gb-swatch" style="color:${colorA}"></i>${nameA}</span>`
    + `<span class="gb-key"><i class="gb-swatch" style="color:${colorB}"></i>${nameB}</span></div>`;
  h += `<div class="gb-plot${wide ? ' gb-wide' : ''}">`
    + '<div class="gb-grid"><span></span><span></span><span></span><span></span></div><div class="gb-groups">'
    + items.map((it) => '<div class="gb-group">'
      + `<div class="gb-bar" style="height:${H(it.a)}%;background:${colorA}"><b class="gb-v">${it.a}</b></div>`
      + `<div class="gb-bar" style="height:${H(it.b)}%;background:${colorB}"><b class="gb-v">${it.b}</b></div>`
      + '</div>').join('')
    + '</div></div>';
  return h + '<div class="gb-xaxis">'
    + items.map((it) => `<span class="gb-x">${it.label}</span>`).join('')
    + '</div>';
}

function versus(a) {
  const items = JSON.parse(a.items || '[]');
  return sharedAxisOk(items) ? versusGrouped(a, items) : versusBars(a, items);
}
