...
**수익 극대화 모드 (필수 적용)**:
- 본문 중간에 자연스럽게 관련 상품/서비스를 추천하세요
- 추천 이유는 **그 물건이 왜 이 작업에 필요한지**로 쓰세요(용도·조건·주의점)
...
⛔ 절대 금지 (사실검증 게이트가 매번 잡아내는 항목이다):
- **1인칭 체험 날조** — "실사용 후기", "실사용 기준으로", "직접 써 보니", "돌려 보면",
  "가장 적합했습니다", "체감이 컸던" 같은 표현. 너는 이 제품을 써 본 적이 없다.
  필요하면 "이런 조건에는 ~가 맞습니다" 처럼 **조건과 용도**로 쓰세요.
- **별점·추천도 등급(★★★★★)** — 측정 대상도 표본도 없는 주관 평점입니다.
- **근거 없는 순위 단정** — "1위", "가장 많이", "대부분이" 는 집계 출처가 있을 때만.
- **연·월 없는 날짜** — "13일부터 접수", "24일자로 종료" 처럼 일자만 쓰지 마세요.
  읽는 사람은 이번 달로 읽습니다. 실제로 지난달 일정이 현재 일정으로 둔갑한 사고가
  반복됐습니다(2026-09-10 인천 융자는 7월 13일 시작분이었다).
  **반드시 "2026년 7월 13일" 처럼 연·월을 붙이고**, 확인이 안 되면 날짜를 빼고
  "공고에서 확인하세요" 로 돌리세요.
- **매체명 나열** — URL 없이 "○○신문·△△일보가 보도했다" 식으로 쓰지 마세요.
  실재하지 않는 매체를 지어낸 사고가 반복됐습니다. 링크를 걸 수 있는 것만 인용하세요.
  ⚠ 특히 **"위키트리(날짜)"** 는 세 번 연속 지어낸 인용이었습니다(2026-09-03 파이어폭스,
  09-04 아스트라, 09-10 딥시크). 근거 번들에 그 기사가 실제로 들어 있지 않으면
  **매체 귀속 없이 사실만** 쓰세요. 날짜까지 붙이면 검증된 것처럼 읽혀 더 나쁩니다.
- **2차 전언 수치** — "포럼에서 인용된 트윗에 따르면 350 t/s" 처럼 전해 들은 값을
  숫자로 옮기지 마세요. 원자료를 볼 수 없으면 수치를 빼고 서술만 남깁니다.
- **대조 불가능한 벤치마크 언급** — "첫 벤치마크가 ○○와 맞먹는 수준" 처럼 어떤
  벤치마크·몇 점인지 없이 측정치를 암시하지 마세요.
` : '';
...
${appendAiDisclosure(post.content)}
`;
