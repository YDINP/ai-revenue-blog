...
대부분의 개발자는 `git log --oneline`과 `git log --graph` 정도에서 멈춥니다. 하지만 Git 히스토리는 <span style="font-size:1.2em;font-weight:800">"언제·왜·누가 이 코드가 이렇게 됐지?"</span>라는 실무의 가장 중요한 질문에 답하는 강력한 도구입니다. 2026년에도 변하지 않는, 그러나 자주 놓치는 활용 10가지를 정리했습니다.
...
<div class="chart-bar" data-title="버그 추적 첫 선택은 bisect (참고용)" data-labels="버그 심은 커밋(bisect),코드 언제 바뀜(-S),커밋 복구(reflog),줄 이력(-L/blame)" data-values="10,9,8,8" data-colors="#009e73,#3b82f6,#f59e0b,#8b5cf6" data-unit="점"></div>
...
Git 히스토리는 '과거 기록'이 아니라 <span style="font-size:1.15em;font-weight:700">디버깅·이해·복구의 도구상자</span>입니다. 이 10가지만 손에 익혀도 "언제·왜 이렇게 됐지?"에 몇 초 만에 답하게 됩니다.
