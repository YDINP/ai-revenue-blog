...
좋은 게임을 만드는 것과 팔리는 게임을 만드는 것은 다른 문제다. 2025~2026년 포스트모템과 스팀 데이터를 종합하면, 인디게임의 실패는 대체로 **세 가지 패턴**으로 수렴한다. 남의 실패에서 배우는 게 가장 싸게 배우는 방법이다. (관련: [2026년 인디게임 수익화 현실](/blog/2026-02-26-game-2026-indie-game-monetization-reality-5-truths/))
...
설문 개발자의 **70% 이상**이 "스코프가 너무 컸다"를 실패 요인으로 꼽았다. 샤워 중 떠오른 아이디어를 계속 추가하다 보면 2년 프로젝트가 4년이 되고, 예산은 타들어가며 초점은 흩어진다.

**회피법**: 베타에 도달하면 **피처 락**을 선언한다. 그 시점에 들어와 있지 않은 기능은, 출시 후 첫 업데이트 전까지 '존재하지 않는' 것으로 취급한다. 범위를 줄이는 것은 타협이 아니라 완성의 조건이다. (초기 도구 선택도 범위에 영향을 준다 — [인디 게임 개발 도구 가이드](/blog/2026-02-11-indie-game-development-tools-2026-guide/) 참고)

## 원인 3. 차별성 부재 — "그거 이미 있는데?"

가시성과 스코프를 통과해도, **한 문장으로 설명되는 훅**이 없으면 위시리스트는 모이지 않는다. 넥스트페스트 데이터에서도 위시리스트의 68~88%는 데모를 안 해본 사람이 캡슐 아트·설명만 보고 누른다. 즉 **첫인상(스토어 페이지)이 게임플레이보다 먼저 팔린다.**
