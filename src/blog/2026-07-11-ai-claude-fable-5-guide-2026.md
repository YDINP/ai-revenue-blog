...
2026년 6월, 앤트로픽이 지금까지 일반에 공개한 모델 중 **가장 강력한 Claude Fable 5**를 내놓았습니다. 'Mythos급'이라는 낯선 표현과 함께 등장했는데, 대체 무엇이 달라졌고 왜 화제인지, 공식 발표 기준으로 정리했습니다.

## Fable 5를 한마디로: '봉인 해제된 상위 티어'

앤트로픽에는 공개 라인업(Haiku·Sonnet·Opus) 위에, 더 강력한 **Mythos 계열**이 있습니다. 그동안은 일반에 열지 않았죠. **Fable 5는 이 Mythos급 능력을 '안전하게 쓸 수 있도록' 다듬어 처음으로 일반 공개한 모델**입니다.
...
강력한 능력에는 책임이 따릅니다. Fable 5는 민감하다고 판단되는 일부 주제의 질문을 **Claude Opus 4.8이 대신 응답**하도록 안전장치를 걸어뒀습니다.

- 이 장치는 **평균 전체 세션의 5% 미만**에서만 작동하도록 조정됨
- 실제로 앤트로픽은 초기 공개 후 가드레일을 더 강화해 **재배포**하기도 했음
...
- **긴·복잡한 작업**에서 특히 강함 (SW·지식노동·비전·과학 SOTA)
