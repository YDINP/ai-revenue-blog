...
faq:
  ...
  - q: "게임 만들면 실제로 얼마 버나?"
    a: "플랫폼마다 답이 다르다. 스팀은 2025년 신작 매출 중앙값이 약 249달러이고, 신작의 66%가 매출 1,000달러를 넘지 못한다. 모바일은 DAU 1만에 유저당 전면 3회·보상형 2회를 가정하면 광고 매출이 월 2만 달러 안팎이지만, DAU를 사는 UA 비용을 빼면 대부분의 프로젝트가 음수가 된다. 웹게임은 DAU 1,000 기준 월 400달러 수준이고, 포털 유입이면 레브셰어로 절반이 깎인다. 결국 남의 벤치마크가 아니라 '판매량 × 가격 × 0.7 × (1−환불률)'과 'DAU × 노출 × eCPM ÷ 1,000' 두 계산식에 자기 숫자를 넣어야 답이 나온다."
---
...
- **모바일**: DAU 1만짜리 캐주얼 게임의 광고 매출은 월 약 2,930만원. 단, DAU를 사는 UA 비용을 빼면 대부분 음수가 된다.
- **웹게임**: DAU 1,000이면 월 약 62만원. 포털 유입이면 레브셰어로 절반이 깎인다.
...
플랫폼마다 답이 다르다. 스팀은 2025년 신작 매출 중앙값이 약 249달러이고, 신작의 66%가 매출 1,000달러를 넘지 못한다. 모바일은 DAU 1만에 유저당 전면 3회·보상형 2회를 가정하면 광고 매출이 월 2만 달러 안팎이지만, DAU를 사는 UA 비용을 빼면 대부분의 프로젝트가 음수가 된다. 웹게임은 DAU 1,000 기준 월 400달러 수준이고, 포털 유입이면 레브셰어로 절반이 깎인다. 결국 남의 벤치마크가 아니라 '판매량 × 가격 × 0.7 × (1−환불률)'과 'DAU × 노출 × eCPM ÷ 1,000' 두 계산식에 자기 숫자를 넣어야 답이 나온다.
