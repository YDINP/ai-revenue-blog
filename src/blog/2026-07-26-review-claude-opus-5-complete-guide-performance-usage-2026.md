...
image:
  url: "https://images.pexels.com/photos/30901568/pexels-photo-30901568.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=650&w=940"
  alt: "디버깅과 코드 리뷰 화면을 보여주는 AI 보조 코딩 클로즈업 — Claude Opus 5 성능 비교 이미지"
faq:
...
**주목할 점은 가격이 그대로라는 것**입니다. 세대가 올라갔지만 입력 $5·출력 $25 per MTok가 유지되므로, 기존 Opus 4.8 워크로드는 모델 ID만 바꿔 성능 개선분을 그대로 받을 수 있습니다.
...
각 벤더가 공개한 API 단가와 컨텍스트는 대조가 가능하지만, 세 모델을 같은 조건에서 측정한 공개 벤치마크는 없습니다. 그래서 아래 표는 **출처가 확인되는 항목만** 담았습니다.
...
즉 **같은 작업이라도 캐싱과 배치를 쓰면 절반 수준까지 내려갑니다**. 반복 프롬프트가 많은 파이프라인이라면 캐싱 설계가 모델 선택보다 비용에 더 큰 영향을 줍니다.
